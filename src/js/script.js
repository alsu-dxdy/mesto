(() => {
  ...
  const userInfo = new UserInfo(editProfileForm,
  ...
  const formValidEditProfile = new FormValidator(document.querySelector('.popup_edit'));
  const formValidAvatar = new FormValidator(document.querySelector('.popup_avatar'));
  ...
  newCardForm.addEventListener('submit', function (event) {
    ...
    api.addCard(newCardForm.elements.name.value, newCardForm.elements.link.value)
      .then((data) => {
        const cardElement = card.create(data.name, data.link, data.likes.length, data.owner._id, data._id);
  ...
  placesList.addEventListener('click', function (event) {
    //debugger;
    if (event.target.classList.contains('place-card__delete-icon_visible')
  ...
  editProfileForm.addEventListener('submit', function (event) {
    ...
    //debugger;
    api.sendServerUserInfo(editProfileForm.username.value, editProfileForm.job.value)
      .then((data) => {
  ...
  formAvatar.addEventListener('submit', function (event) {
    event.preventDefault();
    api.sendServerAvatar(formAvatar.elements.avatar.value)
      .then((data) => {
